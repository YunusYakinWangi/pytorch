...
def _nccl_bandwidth_latency(group_size: int, coll: NCCL_COLL) -> tuple[float, float]:
    ...
    compute_min_saturation_bytes. Heuristics from
    https://github.com/NVIDIA/nccl/blob/master/src/graph/tuning.cc.
    """
    num_gpus_per_node = 8
    ...
    # NOTE: the original code in nccl/src/graph/tuning.cc
    ...
    latency_us += (nsteps - nInterSteps) * intraLat + nInterSteps * interLat  # type: ignore[possibly-undefined]

    return bandwidth_GB_per_s, latency_us
...
def estimate_nccl_collective_runtime_impl(
    ...
    bandwidth_GB_per_s, latency_us = _nccl_bandwidth_latency(group_size, coll)

    tensor_storage_size_GB = tensor_storage_size_bytes / 1024 / 1024 / 1024
    bandwidth_GB_per_ns = bandwidth_GB_per_s / 1e9
