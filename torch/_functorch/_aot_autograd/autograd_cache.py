...
class AOTAutogradCacheDetails(FxGraphHashDetails):
    ...
    def __init__(
        ...
        try:
            ...
            with dynamo_timed("AOTAutogradCache.FxGraphHashDetails.__init__"):
...
def autograd_cache_key(
    ...
    with sanitize_gm_for_cache(gm), dynamo_timed("AOTAutogradCache.autograd_cache_key"):
        try:
            check_cacheable(gm)
            if has_triton_package():
            ...
            with dynamo_timed(
                "AOTAutogradCache.autograd_cache_key.AOTAutogradCacheDetails"
            ):
            ...
            with dynamo_timed("AOTAutogradCache.autograd_cache_key.get_hash"):
            ...
            if log.isEnabledFor(logging.DEBUG):
                with dynamo_timed(
                    "AOTAutogradCache.autograd_cache_key.debug_lines"
                ):
...
class AOTAutogradCache(GuardedCache[GenericAOTAutogradResult[Any, Any]]):
    ...
    **No new guards are generated into the shape env after inductor finishes compiling**, so the guards
    ...
    def _lookup(
        ...
        with dynamo_timed("AOTAutogradCache._lookup"):
            ...
            try:
                with dynamo_timed("AOTAutogradCache.find_guarded_entry"):
                    (
                        entry,
                        pickled_content,
                        guard_info,
                        ...
                        local,
                        remote_cache,
                        AOTAutogradCache.evaluate_guards,
                        hints,
                    )

                if (
                    entry is None
                    and guard_info["cache_status_detailed"] == "guard_miss"
                ):
