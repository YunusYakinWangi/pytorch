...
def _is_multi_output_node(node: fx.Node) -> bool:
    ...
    A `MUST_SAVE` annotation on such a parent should be ignored by the
    partitioner because Inductor's output() expects flat saved values.
    The tensor leaves extracted via `getitem` remain available for the cut.
    """
...
def force_save_multi_output_must_save_ops(joint_module: fx.GraphModule) -> None:
    ...
    MUST_SAVE, we cannot save the parent value directly because Inductor's
    saved-values path expects flat tensor leaves. Propagate MUST_SAVE to the
    tensor leaves extracted via getitem so the partitioner can honor the
    policy without re-running the parent op in backward.
    """
    for node in joint_module.graph.nodes:
        if (
...
def solve_min_cut(
    ...
    def should_ban_recomputation(node: fx.Node) -> str | None:
        ...
        # parents — Inductor's output() rejects tuples in saved_values.
        if (
            node.meta.get("recompute", None) == CheckpointPolicy.MUST_SAVE
            and not _is_multi_output_node(node)
        ):
