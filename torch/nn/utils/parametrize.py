                ...
                if is_opaque_value(originali):
                    if not is_opaque_reference_type(type(originali)):
                        raise ValueError(
                            ...
                            f"{type(originali).__name__}."  # pyrefly: ignore[missing-attribute]
                        )
                    setattr(self, f"original{i}", originali)
                elif isinstance(originali, Tensor):
        ...
        with torch.no_grad():
            ...
            if self.is_tensor:
                ...
                for i, tensor in enumerate(value):
                    ...
                    if is_opaque_value(tensor) and is_opaque_reference_type(
                        type(tensor)
                    ):
